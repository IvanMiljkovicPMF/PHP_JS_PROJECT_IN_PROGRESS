<?php
    ...
    class Db{
        ...
        { try 
            ...
            {
                try { $sql = "SELECT * FROM korisnik WHERE korisnicko_ime='$user' and sifra='$pass'"; 
            ...
            {
                try { $sql = "SELECT * FROM tip_korisnika WHERE id=$id"; 
                $pdo_izraz = $this->dbh->query($sql);
                $obj = $pdo_izraz->fetch(PDO::FETCH_ASSOC); 
                return $obj; } 
                catch(PDOException $e) 
            ...
            public function izmeniKolicinu($id) 
                    { try { $sql = "UPDATE proizvod SET kolicina=:kolicina WHERE id=:id"; 
                    $pdo_izraz = $this->dbh->prepare($sql); 
